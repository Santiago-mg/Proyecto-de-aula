...
describe('updateOrderStatus', () => {
  ...
  // El caso de uso valida la transición: desde PENDING solo se puede pasar a
  ...
  it('updates the status of an existing order', async () => {
    ...
    const result = await updateOrderStatus(mockRepo, order.id, 'CONFIRMED');
    ...
    expect(mockRepo.updateStatus).toHaveBeenCalledWith(order.id, 'CONFIRMED');
  });

  it('rejects with 400 an invalid transition (PENDING to SHIPPED)', async () => {
    const order = makeCreatedOrder(makeOrderData(), 'u1');
    mockRepo.findById.mockResolvedValueOnce(order);

    await expect(
      updateOrderStatus(mockRepo, order.id, 'SHIPPED'),
    ).rejects.toMatchObject({ statusCode: 400 });

    expect(mockRepo.updateStatus).not.toHaveBeenCalled();
  });

  it('rejects with 400 when the order is already in that status', async () => {
    const order = makeCreatedOrder(makeOrderData(), 'u1');
    mockRepo.findById.mockResolvedValueOnce(order);

    await expect(
      updateOrderStatus(mockRepo, order.id, 'PENDING'),
    ).rejects.toMatchObject({ statusCode: 400 });
